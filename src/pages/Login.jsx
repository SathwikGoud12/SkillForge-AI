    ...
    <div style={{ height: '100vh', width: '100vw', display: 'flex', fontFamily: "'Inter','Segoe UI',sans-serif", overflow: 'hidden' }}>
      ...
      <style>{`
        @media (max-width: 768px) {
          ...
          .sf-login-right { padding: 28px 20px !important; }
        }
      `}</style>

      {/* ── LEFT PANEL (hidden on mobile) ── */}
      <div className="sf-login-left" style={{ flex: '0 0 52%', background: '#000', position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 48 }}>
      ...
      <div className="sf-login-right" style={{ flex: 1, background: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 48, overflowY: 'auto' }}>
        <div style={{ width: '100%', maxWidth: 380 }}>
          ...
          <button onClick={handleGoogleLogin} disabled={googleLoading}
            ...
            onMouseEnter={e => { if (!googleLoading) { e.currentTarget.style.background = '#f8fafc'; e.currentTarget.style.borderColor = '#d1d5db'; } }}
            onMouseLeave={e => { e.currentTarget.style.background = '#fff'; e.currentTarget.style.borderColor = '#e5e7eb'; }}>
            {googleLoading ? (
              <>
                <div style={{ width: 18, height: 18, border: `2px solid #e5e7eb`, borderTopColor: ACCENT2, borderRadius: '50%', animation: 'spin 0.7s linear infinite' }} />
