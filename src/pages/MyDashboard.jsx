          ...
          return (
            <Link key={item.path} to={item.path}>
              <motion.div
                whileHover={{ x: 6 }}
                whileTap={{ scale: 0.98 }}
                className={`relative flex items-center justify-between px-5 py-4 rounded-[1.25rem] group transition-all duration-300 ${
                  active ? "bg-gradient-to-r from-indigo-50 to-violet-50 text-indigo-700 shadow-sm" : "text-slate-500 hover:bg-slate-50"
                }`}
              >
                <div className="flex items-center gap-4">
                  <div className={`w-11 h-11 rounded-2xl flex items-center justify-center transition-all duration-300 ${
                    active ? `bg-gradient-to-br ${item.gradient} text-white shadow-lg shadow-indigo-100` : "bg-slate-50 group-hover:bg-white group-hover:shadow-md"
                  }`}>
      ...
      <div className="p-8 border-t border-slate-50 bg-slate-50/30">
        ...
        >
          <AnimatePresence mode="wait">
            {isLoggingOut ? (
              <motion.div key="lo" className="flex items-center gap-3" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
              ...
              <motion.div key="idle" className="flex items-center gap-3" initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
    ...
    <div className="flex h-screen bg-white overflow-hidden font-inter">
      ...
      {isMobile && (
        <button
          ...
          className="fixed top-4 left-4 z-50 w-11 h-11 rounded-2xl flex items-center justify-center shadow-xl bg-gradient-to-br from-indigo-600 to-violet-700"
        >
      ...
      <AnimatePresence>
        {isMobile && mobileOpen && (
          <motion.aside
            ...
            className="fixed left-0 top-0 h-full w-80 bg-white border-r border-slate-100 flex flex-col z-50 shadow-2xl"
          >
            <button
              onClick={() => setMobileOpen(false)}
              className="absolute top-4 right-4 z-50 w-8 h-8 rounded-xl bg-slate-100 flex items-center justify-center"
            >
      ...
      {!isMobile && (
        <motion.aside
          ...
          className="w-80 bg-white border-r border-slate-100 flex flex-col relative z-50 transition-all duration-300 flex-shrink-0"
        >
          <SidebarBody />
        </motion.aside>
      )}
      ...
      <main className="flex-1 overflow-y-auto bg-slate-50 relative">
        ...
        <div className="relative z-10 max-w-[1500px] mx-auto p-6 md:p-12 lg:p-16">
          {isMobile && <div className="h-14" />}  {/* spacer for hamburger */}
          <Outlet />
        </div>
      </main>
    </div>
